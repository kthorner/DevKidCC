...
DKCC <- function(seurat, threshold = 0.7, max.iter = 1) {
  ...
  #}
  old.seurat <- seurat
  seurat <- CreateSeuratObject(old.seurat@assays$SCT@counts, meta.data = md)
  ...
  npcs <- npcs %>% SCTransform()
  npcs$SCT_snn_res.0.5 <- NULL
  if (ncol(npcs)>50) {
    npcs <- npcs %>%
  ...
  npcs$orig.ident <- "all"
  markers <- DevKidCC::GeneSummary(npcs, identity = "orig.ident", split.by = "SCT_snn_res.0.5", features = c("PAX2"))
  pax2null <- (markers %>% filter(pct.exp<33))$Component
  if (length(pax2null)>0){
    names <- colnames(npcs[, npcs$SCT_snn_res.0.5 %in% pax2null])
